...
/*
 * AnnaMike loves Physics! Recently, he learned about Elastic Collisions. A
 * perfectly
 * elastic collision is the one where the total momentum of the colliding bodies
 * does
 * not change after the collision. A very important property of elastic
 * collisions,
 * that AnnaMike observed is that, if the 2 colliding bodies have equal masses,
 * their
 ...
 * 
 * AnnaMike was immensely happy to see the large number of collisions that
 * happened
 * on the board. But, the balls soon started to fall off the board (from both
 * the ends),
 * and finally there was no ball left. He is now interested to know : At what
 * time
 * did the last ball fall? Will you help him?
 * 
 * NOTE:
 * -----
 * - A ball will fall off the board when it is moving towards left and reaches x
 * = 0,
...
public class ElasticCollisions {
    public static void main(String[] args) {
        ...
        System.out.println(lastBallFallTime(pos, vel, l));
        sc.close();
    }

    private static int lastBallFallTime(int[] positions, int[] velocities, int length) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (vel > 0) {
                maxTime = Math.max(maxTime, (length - pos) / vel);
            } else {
                maxTime = Math.max(maxTime, (pos - 0) / Math.abs(vel));
            }
        }
