...
// 45A courier company has to deliver N items with different heights.
...
public class DeliverItems {
    public static void main(String[] args) {
        ...
        System.out.println(findMaxItems(items, rooms));

        sc.close();
    }

    private static int findMaxItems(int[] items, int[] rooms) {
        boolean[] taken = new boolean[rooms.length];
        ...
        newArr[0] = rooms[0];
        for (int i = 1; i < n; i++) {
            newArr[i] = Math.min(newArr[i - 1], rooms[i]);
        }

        return helper(items, newArr, 0, taken);
    }

    private static int helper(int[] items, int[] rooms, int idx, boolean[] taken) {
        if (idx == items.length)
            return 0;

        int sum = 0;
        for (int i = rooms.length - 1; i >= 0; i--) {
            if (!taken[i] && items[idx] <= rooms[i]) {
                taken[i] = true;
                sum += 1;
                break;
            }
        ...
        return sum + helper(items, rooms, idx + 1, taken);
    }
}