...
class Val implements Comparable<Val> {
    ...
    public Val(double p, int v, int o) {
        this.priority = p;
        this.val = v;
        this.occ = o;
    }

    public int compareTo(Val v1) {
        if (this.priority != v1.priority)
            return (int) v1.priority - (int) this.priority;
        else
            return v1.val - this.val;
    }

    public String toString() {
        return "Priority: " + priority + " val: " + val + " occ: " + occ;
    }
}
...
public class TreasureHunt {
    public static void main(String[] args) {
        ...
        int[] res = calculateSums(boxes, n, k, x, f);
        System.out.println(Arrays.toString(res));

        sc.close();
    }

    private static int[] calculateSums(int[] nums, int n, int k, int x, int f) {
        int[] priorityScores = new int[n - k + 1];
        int idx = 0;
        ...
        while (j < n) {
            ...
            int len = j - i + 1;
            if (len == k) {
                // System.out.println("i: " + i + " j: " + j + " freq: " + freq);
                priorityScores[idx++] = getPriorityScore(freq, f, x);
            }
    ...
    private static int getPriorityScore(Map<Integer, Integer> freq, int f, int x) {
        PriorityQueue<Val> pq = new PriorityQueue<>();

        for (Map.Entry<Integer, Integer> e : freq.entrySet()) {
            double priority = e.getValue() * Math.pow(e.getKey(), f);
            Val newVal = new Val(priority, e.getKey(), e.getValue());
        ...
        while (!pq.isEmpty() && x-- > 0) {
            ...
            // System.out.println("Popped class: " + popped);
            priorityScore += val * occ;
        }

        // System.out.println("Summed Priority Score: " + priorityScore);
