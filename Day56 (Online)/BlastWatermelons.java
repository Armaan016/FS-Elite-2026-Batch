...
public class BlastWatermelons {
    public static void main(String[] args) {
        ...
        System.out.println(countShots(melons));
        sc.close();
    }

    private static int countShots(int[][] melons) {
        Arrays.sort(melons, (a, b) -> a[1] - b[1]);

        int end = melons[0][1];
        int shots = 1;
        for (int i = 1; i < melons.length; i++) {
            if (melons[i][0] <= end) {
                // end = Math.max(end, melons[i][1]); 
            } else {
                end = melons[i][1];
                shots++;
            }
