...
// online booking of the slots to collect the vouchers. And cost of each slot
// may
...
// - The customer is allowed to collect the vouchers one at a time and not
// immediately.
...
public class ShoppingVouchers {
    public static void main(String[] args) {
        ...
        System.out.println(findMinRefund(s, penalty));

        sc.close();
    }

    private static int findMinRefund(String s, int[] penalty) {
        int n = s.length();
        ...
        for (int i = 0; i < n; i++) {
            ...
            while (k < n && s.charAt(k) == s.charAt(i)) {
                maxCost = Math.max(maxCost, penalty[k]);
                k++;
            }

            // System.out.println("maxCost: " + maxCost + " for i: " + i);
            cost += maxCost;
            i = k - 1;
        }
