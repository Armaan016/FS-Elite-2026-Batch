...
public class ParcelDistances {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < p; i++)
            parcels[i] = new int[] { sc.nextInt(), sc.nextInt() };

        System.out.println(findMinDistances(emps, parcels));

        sc.close();
    }

    private static List<Integer> findMinDistances(int[] emps, int[][] parcels) {
        ...
        for (int[] parcel : parcels) {
            ...
            for (int idx : map.getOrDefault(comp, new ArrayList<>())) {
                minDist = Math.min(minDist, Math.abs(pos - idx));
            }
