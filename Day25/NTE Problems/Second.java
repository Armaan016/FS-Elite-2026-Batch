...
public class Second {
    public static void main (String[] args) {
        ...
        int lvls[] = new int[n];
        for(int i = 0; i < n; i++) lvls[i] = sc.nextInt();
        
        System.out.println(findMinSum(lvls));

        sc.close();
    }
    
    private static int findMinSum(int[] lvls) {
        int n = lvls.length;
        // Arrays.sort(lvls);
        ...
        for(int i = 0; i < n; i++) {
            if(map.containsKey(lvls[i])) {
                lvls[i]++;
                while(map.containsKey(lvls[i])) {
                    lvls[i]++;
                }
            }
            
            map.put(lvls[i], map.getOrDefault(lvls[i], 0) + 1);
        }
