...
public class EmphaticPronunciation {
    public static void main(String[] args) {
        ...
        System.out.println(checkWords(words, s));

        sc.close();
    }

    public static int checkWords(String[] words, String s) {
        int count = 0;
        for (String word : words) {
            if (checkValidity(word, s))
                count++;
        }
    ...
    private static boolean checkValidity(String word, String emph) {
        int i = 0, j = 0;
        while (i < word.length() && j < emph.length()) {
            if (word.charAt(i) != emph.charAt(j))
                return false;

            int l1 = checkChars(word, i);
            int l2 = checkChars(emph, j);
        ...
        return i == word.length() && j == emph.length();
    }

    private static int checkChars(String word, int idx) {
        int i = idx;
        while (i < word.length() && word.charAt(i) == word.charAt(idx)) {
            i++;
        }
