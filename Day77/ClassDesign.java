...
class OrderProcessor implements Runnable {
    ...
    public OrderProcessor(IndexedOrder io, OrderValidator v, Map<Integer, String> map) {
    ...
    public void run() {
        ...
        try {
            ...
            Thread.sleep(1000);
            outputMap.put(indexedOrder.index, "Order accepted: " + order.customerName + " ordered " + order.quantity
                    + " of " + order.productName);
        } catch (Exception e) {
            // TODO: Save failure message
            outputMap.put(indexedOrder.index, "Order failed: " + order.customerName + " - " + e.getMessage());
        }
    }
}
