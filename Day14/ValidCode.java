...
// For instance, the message "substitution" can be encoded in various ways, such
// as:

// • "s10n" (keeping 's', replacing the next 10 characters with 10, and ending
// with 'n')
// • "sub4u4" (keeping "sub", replacing 4 characters, then 'u', and replacing 4
// more characters)
...
// Explanation: "internationalization" can be encoded as "i12iz4n" by keeping
// 'i',
// replacing 12 letters, keeping "iz", replacing 4 letters, and then ending with
// 'n'.
...
public class ValidCode {
    public static void main(String[] args) {
        ...
        System.out.println(checkValidity(original, encoded));

        sc.close();
    }

    private static boolean checkValidity(String original, String encoded) {
        int i = 0, j = 0;
        int m = original.length(), n = encoded.length();

        while (j < n) {
            // System.out.println("Comparing i: " + i + " j: " + j);
            if (original.charAt(i) == encoded.charAt(j)) {
                i++;
                j++;
            } else if (Character.isDigit(encoded.charAt(j))) {
                int len = 0;
                if (encoded.charAt(j) == '0')
                    return false;
                while (j < n && Character.isDigit(encoded.charAt(j))) {
                    len = len * 10 + (encoded.charAt(j) - '0');
                    j++;
                }
