...
async function processBooking(userId) {
    ...
    try {
        ...
        const [fetchedMovies, checkedUserDetails] = await Promise.all([
            fetchAvailableMovies(),
            checkUserDetails(userId)
        ])

        // console.log(fetchedMovies);
        console.log(checkedUserDetails);

        // Sequential Activities: Choose movie and select showtime

        const chosenMovie = await chooseMovie(fetchedMovies);
        console.log(chosenMovie);

        const selectedShowtime = await selectShowtime(chosenMovie);
        ...
        const [emailConfirmation, smsConfirmation] = await Promise.all([
