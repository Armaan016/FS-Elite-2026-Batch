...
public class MovieMarathon {
    public static void main(String[] args) {
        ...
        System.out.println(maxValidRating(movies, restricted, k));

        sc.close();
    }

    private static int maxValidRating(int[] movies, Set<Integer> restricted, int k) {
        int n = movies.length;
        ...
        int i = 0, j = 0, rating = 0;
        while (j < n) {
            if (restricted.contains(movies[j]) || unique.contains(movies[j])) {
                j = j + 1;
                i = j;
                rating = 0;
                unique.clear();
        ...
        return maxRating == Integer.MIN_VALUE ? -1 : maxRating;
    }
}